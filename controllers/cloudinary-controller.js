...
const deleteTileMap = async (req, res) => {
    ...
    const response2 = await cloudinary.v2.uploader.destroy(deleteTileMap).catch(error => { console.log("Tilemap not found") })
    const response3 = await cloudinary.v2.api.delete_resources_by_tag(tilemap_id).catch(error => { console.log("Tilemap id not found") });
    const response = await cloudinary.v2.api.delete_folder(searchFolder).catch(error => {console.log("Folder not found")})
    return res.status(200).json({
        _id: tilemap_id
    })
